    ...
    <main>
                                    ...
                                    ?>
                                        <div class="card">
                                            <div class="card-header">
                                                ...
                                                <?php
                                                foreach ($data2 as $subject) {
                                                    if ($section['id'] == $subject['section_id'] && $subject['is_assign'] == 0) {
                                                ?>
                                                        <div class="form-check float-end mt-1">
                                                            <input class="form-check-input" type="checkbox" name="check_all" value="1" id="check_<?php echo $section['id']; ?>">
                                                            <label class="form-check-label" for="check_<?php echo $section['id']; ?>">
                                                                check all
                                                            </label>
                                                        </div>
                                                <?php
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form class="postForm" method="POST" action="<?php echo ROOT; ?>manage_faculty/update">
                        <div class="modal-body">
                            <input class="form-control d_subjects" type="text" name="d_subjects" readonly hidden>
                            <input class="form-control id" type="text" name="id" readonly hidden>
                            <input class="form-control e_teacher_old" type="text" name="e_teacher_old" readonly hidden>
                            <div class="row mt-3 gap-3">
                                <div class="col-12 input-group">
                                    <span class="input-group-text w-25">
                                        Teacher:
                                    ...
                                    ?>
                                        <div class="card">
                                            <div class="card-header">
                                                ...
                                                ?>
                                                        <div class="form-check float-end mt-1">
                                                            <input class="form-check-input" type="checkbox" name="check_all" value="1" id="e_check_<?php echo $section['id']; ?>">
                                                            <label class="form-check-label" for="e_check_<?php echo $section['id']; ?>">
                                                                check all
                                                            </label>
                                                        </div>
                                                <?php
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                $('input[type="checkbox"][name="subject[]"]').on('change', function() {
                    let parentCard = $(this).closest('.card');
                    let allSubjects = parentCard.find('input[type="checkbox"][name="subject[]"]');
                    let checkAllCheckbox = parentCard.find('input[type="checkbox"][id^="check_"]');

                    if (allSubjects.length === allSubjects.filter(':checked').length) {
                        checkAllCheckbox.prop('checked', true);

                        $('.card').not(parentCard).find('input[type="checkbox"][id^="check_"]').each(function() {
                            $(this).prop('checked', false).prop('disabled', true);
                        });

                        $('.card').not(parentCard).find('input[type="checkbox"][name="subject[]"]').prop('checked', false);
                        $('.card').not(parentCard).find('input[type="checkbox"][name="subject[]"]').prop('disabled', true);
                    } else {
                ...
                $('input[type="checkbox"][id^="check_"]').on('change', function() {
                    let isChecked = $(this).is(':checked');
                    let parentCard = $(this).closest('.card');

                    parentCard.find('input[type="checkbox"][name="subject[]"]').prop('checked', isChecked);

                    if (isChecked) {
                        $('.card').not(parentCard).find('input[type="checkbox"]').each(function() {
                            $(this).prop('checked', false).prop('disabled', true);
                        });
                    } else {
                        $('.card').find('input[type="checkbox"]').prop('disabled', false);
                ...
                $('input[type="checkbox"][name="e_subject[]"]').on('change', function() {
                    let parentCard = $(this).closest('.card');
                    let allSubjects = parentCard.find('input[type="checkbox"][name="e_subject[]"]');
                    let checkAllCheckbox = parentCard.find('input[type="checkbox"][id^="e_check_"]');
                    ...
                    if ($(this).is(':checked') && allSubjects.length === allSubjects.filter(':checked').length) {
                        $('.card').not(parentCard).find('input[type="checkbox"]').prop('checked', false);
                        $('.card').not(parentCard).find('input[type="checkbox"]').prop('disabled', true);
                    } else {
                        $('.card').find('input[type="checkbox"]').prop('disabled', false);
                    }
                });

                $('input[type="checkbox"][id^="e_check_"]').on('change', function() {
                    let isChecked = $(this).is(':checked');
                    let parentCard = $(this).closest('.card');

                    parentCard.find('input[type="checkbox"][name="e_subject[]"]').prop('checked', isChecked);
            ...
            $('input[type="checkbox"][name="e_subject[]"]').on('change', function() {
                let parentCard = $(this).closest('.card');
                updateCheckAllState(parentCard);
            });

            function updateCheckAllState(cardElement) {
                let allSubjects = cardElement.find('input[type="checkbox"][name="e_subject[]"]');
                let checkAllCheckbox = cardElement.find('input[type="checkbox"][id^="e_check_"]');
                checkAllCheckbox.prop('checked', allSubjects.length === allSubjects.filter(':checked').length);
                updateDisableState();
            }

            function updateDisableState() {
                if ($('input[type="checkbox"][id^="e_check_"]:checked').length) {
                    $('.card').each(function() {
                        if (!$(this).find('input[type="checkbox"][id^="e_check_"]').is(':checked')) {
                            $(this).find('input[type="checkbox"][name="e_subject[]"], input[type="checkbox"][id^="e_check_"]').prop('disabled', true);
                        }
                    });
                } else {
                    $('input[type="checkbox"][name="e_subject[]"], input[type="checkbox"][id^="e_check_"]').prop('disabled', false);
                }
            }
            ...
            $("#addModal").on("hidden.bs.modal", function(event) {
                ...
                $('.card').find('input[type="checkbox"][name="subject[]"]').prop('checked', false);
                $('.card').find('input[type="checkbox"][name="subject[]"]').prop('disabled', false);
                $('.card').find('input[type="checkbox"][name="e_subject[]"]').prop('checked', false);
                $('.card').find('input[type="checkbox"][name="e_subject[]"]').prop('disabled', false);
                $('.card').find('input[type="checkbox"][name="check_all"]').prop('checked', false);
                $('.card').find('input[type="checkbox"][name="check_all"]').prop('disabled', false);
            });
